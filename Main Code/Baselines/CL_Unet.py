...
class PeriodicPadding(nn.Module):
    def __init__(self, pad_width, pad_height):
        ...
        self.pad_width = pad_width  # Periodic padding in longitude
        self.pad_height = pad_height  # Zero padding in latitude

    def forward(self, x):
        # Apply periodic padding in the x-axis (longitude)
        x = torch.cat([x[..., -self.pad_width:], x, x[..., :self.pad_width]], dim=-1)
        
        # Apply zero padding in the y-axis (latitude)
        x = F.pad(x, (0, 0, self.pad_height, self.pad_height), mode="constant", value=0)
...
class UNetPDEArena(nn.Module):
    def __init__(self, in_channels=141, hidden_dim=64, channel_mults=[1, 2, 2], num_blocks=2, dropout=0.1, final_channels = 1):
        super().__init__()
        self.padding = PeriodicPadding(pad_width=3, pad_height=3)
        
        self.initial_conv = nn.Conv2d(in_channels, hidden_dim, kernel_size=7, stride=1, padding=0, bias=False)
        ...
        self.final_conv = nn.Conv2d(current_channels, final_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        x = x.permute(0, 3, 1, 2)  # (batch, features, lat, lon) -> (batch, channels, lat, lon)
        x = self.padding(x)
        ...
        x = self.final_conv(x)
        x = x.permute(0, 2, 3, 1)  # (batch, channels, lat, lon) -> (batch, lat, lon, features)
        x = x.squeeze(-1) 
...
model = UNetPDEArena(final_channels = out_steps)
model = model.to(device)
optimizer = optim.AdamW(model.parameters(), lr=5e-4, weight_decay=1e-5)
criterion = LatitudeWeightedRMSELoss(latitude_tensor)

best_model_path = f'/home/anonymus/Weather Prediction/Code/Baselines/Checkpoints/unet_best_model{pred_var}.pth'
last_model_path = f'/home/anonymus/Weather Prediction/Code/Baselines/Checkpoints/unet_last_model{pred_var}.pth'
...
def save_metrics_to_json(metrics_per_step, out_steps, pred_var, gap, filename="/home/anonymus/Weather Prediction/Code/Main Code/Results/Direct Forecasting Results/CL_Unet_Direct_Forecasting_result.json"):
