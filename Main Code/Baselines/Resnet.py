...
class PeriodicPadding2D(nn.Module):
    """
    Custom padding layer: periodic padding for longitude (W) and zero padding for latitude (H).
    """
    def __init__(self, pad):
        super(PeriodicPadding2D, self).__init__()
        self.pad = pad

    def forward(self, x):
        # Periodic padding for longitude (dim=3, width)
        x = F.pad(x, (self.pad, self.pad, 0, 0), mode="circular")
        # Zero padding for latitude (dim=2, height)
...
class ResidualBlock(nn.Module):
    """
    Residual block with:
    Periodic Padding -> Conv2D -> LeakyReLU -> BatchNorm -> Dropout -> 
    Periodic Padding -> Conv2D -> BatchNorm -> Residual Connection.
    """
    def __init__(self, hidden_dim, kernel_size, dropout_rate):
        super(ResidualBlock, self).__init__()
        self.padding_layer = PeriodicPadding2D((kernel_size - 1) // 2)  # Compute required padding

        self.conv1 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size, stride=1, padding=0)  # External padding used
        self.leaky_relu = nn.LeakyReLU(0.3)
        self.batch_norm1 = nn.BatchNorm2d(hidden_dim)
        self.dropout = nn.Dropout2d(dropout_rate)

        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size, stride=1, padding=0)  # External padding used
        self.batch_norm2 = nn.BatchNorm2d(hidden_dim)

    def forward(self, x):
        identity = x
        out = self.padding_layer(x)  # Apply periodic + zero padding before Conv
        out = self.conv1(out)
        out = self.leaky_relu(out)
        out = self.batch_norm1(out)
        out = self.dropout(out)

        out = self.padding_layer(out)  # Apply periodic + zero padding again
        out = self.conv2(out)
...
class ResNet(nn.Module):
    """
    ResNet with periodic padding in longitude and zero padding in latitude.
    """
    def __init__(self, input_channels=141,hidden_dim = 128, num_blocks = 28, kernel_size = 3, dropout_rate = 0.1, final_channels=1):
        super(ResNet, self).__init__()
        self.initial_padding = PeriodicPadding2D((7 - 1) // 2)  # Ensure correct padding for initial conv

        # Initial convolution (7x7 kernel)
        self.initial_conv = nn.Conv2d(input_channels, hidden_dim, kernel_size=7, stride=1, padding=0)  # External padding used
        self.leaky_relu = nn.LeakyReLU(0.3)
        self.batch_norm = nn.BatchNorm2d(hidden_dim)

        # Residual blocks
        self.residual_blocks = nn.Sequential(
            *[ResidualBlock(hidden_dim, kernel_size, dropout_rate) for _ in range(num_blocks)]
        )

        # Final padding before last conv
        self.final_padding = PeriodicPadding2D((kernel_size - 1) // 2)
        self.final_conv = nn.Conv2d(hidden_dim, final_channels, kernel_size=kernel_size, stride=1, padding=0)  

    def forward(self, x):
        x = x.permute(0, 3, 1, 2)  # Convert (B, H, W, C) → (B, C, H, W)
        x = self.initial_padding(x)  # Apply padding before first conv
        x = self.initial_conv(x)
        x = self.leaky_relu(x)
        x = self.batch_norm(x)
        x = self.residual_blocks(x)
        x = self.final_padding(x)  # Apply padding before last conv
        x = self.final_conv(x)

        return x.squeeze(1)  # Remove channel dim to get (batch_size, lat, lon)
...
class LatitudeWeightedRMSELoss(nn.Module):
    def __init__(self, latitude_grid):
        ...
        # Convert latitude grid from degrees to radians
        self.weights = torch.cos(torch.tensor(latitude_grid * torch.pi / 180)).to(device)

    def forward(self, predictions, targets):
        squared_error = F.mse_loss(predictions, targets, reduction='none')
        weighted_squared_error = squared_error * self.weights.unsqueeze(0)  # Unsqueeze for batch dimension
        mean_weighted_squared_error = weighted_squared_error.mean()
        return torch.sqrt(mean_weighted_squared_error)
...
model = ResNet(input_channels = 141, hidden_dim = 128, num_blocks = 28, kernel_size = 3, dropout_rate = 0.1, final_channels=out_steps)
model = model.to(device)
optimizer = optim.AdamW(model.parameters(), lr=5e-4, weight_decay=1e-5)
criterion = LatitudeWeightedRMSELoss(latitude_tensor)

best_model_path = f'/home/anonymus/Weather Prediction/Code/Baselines/Checkpoints/resnet_best_model{pred_var}.pth'
last_model_path = f'/home/anonymus/Weather Prediction/Code/Baselines/Checkpoints/resnet_last_model{pred_var}.pth'
...
def save_metrics_to_json(metrics_per_step, out_steps, pred_var, gap, filename="/home/anonymus/Weather Prediction/Code/Main Code/Results/Direct Forecasting Results/Resnet_Direct_Forecasting_result.json"):
