...
def evaluate_climatology_model(data_dir, pred_var, gap, start_year, end_year, eval_years, timestamps_per_day):
    ...
    latitude_grid = np.array(latitudes).reshape(32, 64)
    ...
    # Step 3: Build climatology dictionary
    print("Building climatology...")
    climatology_dict = {}
    for t in range(len(dates) - gap):
        future_dt = dates[t + gap]
        if future_dt.year in eval_years:
            continue
        key = (future_dt.month, future_dt.day, future_dt.hour)
        if key not in climatology_dict:
            climatology_dict[key] = []
        climatology_dict[key].append(data[t])
    for key in climatology_dict:
        climatology_dict[key] = np.mean(np.stack(climatology_dict[key], axis=0), axis=0)

    # Step 4: Get predictions and targets
    ...
    for t in range(len(dates) - gap):
        ...
        if key not in climatology_dict:
            continue
        preds.append(climatology_dict[key])
        targets.append(data[t + gap])

    preds = np.stack(preds, axis=0).reshape(-1, 32, 64)
    targets = np.stack(targets, axis=0).reshape(-1, 32, 64)
...
def save_metrics_to_json(metrics_per_step, out_steps, pred_var, gap, filename="/home/anonymus/Weather Prediction/Code/Main Code/Results/Direct Forecasting Results/Climatology_Direct_Forecasting_result.json"):
