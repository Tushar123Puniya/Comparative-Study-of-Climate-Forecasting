...
# parser.add_argument("--out_steps", type=int, required=True, help="Number of output steps")
# parser.add_argument("--gap", type=int, required=True, help="Gap between input and output")
# parser.add_argument("--pred_var", type=str, nargs='+', required=True, help="List of prediction variables")
# args = parser.parse_args()

# pred_var = args.pred_var[0]
# in_steps = 3
...
pred_var = "z_500"
in_steps = 3
out_steps = 1         #keep this always 1 as it is discrete forecasting
gap = 1               #this represents the gap at which you want to forecast, starting with 1 (for imeediate forecasting)
...
model = UNet(final_channels=out_steps,lat=lat,lon=lon)
model = model.to(device)
optimizer = optim.AdamW(model.parameters(), lr=5e-4, weight_decay=1e-5)
criterion = LatitudeWeightedRMSELoss(latitude_tensor)

best_model_path = f'/home/anonymus/Weather Prediction/Code/Main Code/Checkpoints/discrete_unet_sattn2_best_model{pred_var}.pth'
last_model_path = f'/home/anonymus/Weather Prediction/Code/Main Code/Checkpoints/discrete_unet_sattn2_last_model{pred_var}.pth'
...
def save_metrics_to_json(metrics_per_step, out_steps, pred_var, gap, filename="/home/anonymus/Weather Prediction/Code/Main Code/Results/Direct Forecasting Results/Unet_Direct_Forecasting_result.json"):
